...
- list로 구현하는데 위의 그림을 예시로 들면 min_heap = $[10,15,30,40,50,100,40]$으로 저장되며 push 및 pop을 결정할 때 이진트리 형태로 결정하게 된다.
...
값을 추가할 때마다 heapify_up이 실행되어 Max Heap 속성을 유지합니다:
push(10) → [10]           (루트: 10)
push(5)  → [10, 5]        (10이 루트, 5가 자식)
push(20) → [20, 5, 10]    (20이 10보다 크므로 교환됨!)
push(15) → [20, 15, 10, 5] (15가 5보다 크므로 교환됨!)
push(30) → [30, 20, 10, 5, 15] (30이 20보다 크므로 루트로 올라감!)
핵심: 새 값이 부모보다 크면 위로 올라가며, 루트가 항상 최대값입니다.

### 2. pop() - 최대값 제거 과정

최대값을 제거할 때는 heapify_down이 실행됩니다:
초기: [30, 20, 10, 5, 15]
pop() → 30 제거
       마지막 값(15)을 루트로 이동: [15, 20, 10, 5]
       heapify_down 실행: 15와 자식(20, 10) 비교
       → 20이 더 크므로 교환: [20, 15, 10, 5]
핵심: 루트를 제거한 후 마지막 값을 루트로 올리고, 자식과 비교하며 아래로 내려가며 정렬합니다.

### 3. Max Heap의 특징
- 루트(인덱스 0)가 항상 최대값
- 부모는 항상 자식보다 크거나 같음
- 배열로 구현: 부모 = $(i-1)//2$, 왼쪽 자식 = $2*i+1$, 오른쪽 자식 = $2*i+2$