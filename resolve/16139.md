 # 인간-컴퓨터 상호작용
Created at : 2025-11-30 22:45
[baekjoon_16139](https://www.acmicpc.net/problem/16139)
### 조건
- 시간 : 1
- 메모리 : 256
### 문제
...
for i in range(len(S)):
    if tmp_ret.get(S[i]):
        ...
        tmp_ret[S[i]] = 1
    ret.append(tmp_ret.copy())

q = int(input())
for _ in range(q):
    a,l,r = sys.stdin.readline().strip().split()
    if not ret[int(r)].get(a):
        print("0")
    elif int(l) ==0 or not ret[int(l)-1].get(a):
        print(ret[int(r)].get(a))
    else:
        print(ret[int(r)][a]-ret[int(l)-1][a])
```
...
- Analyze
	- 누적해서 알파벳의 합들을 각기 입력 받은 문자열 마다 추가해 주었다.
		![[Pasted image 20251130232734.png]]
	- 그리고 이후, 입력받은 문자와 범위를 빼주어서 결과를 도출해 주었다.
	- 추가로, 2만개의 입력을 받기에 input이 아닌 sys.stdin.readline()을 이용하였다.
	- Python3 : 100점 (876ms)
	- Pypy3 : 100점 (840ms)
### 알고리즘 분류
- 